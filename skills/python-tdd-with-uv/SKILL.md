...
description: Test-driven development in Python using uv as the package manager. Covers the red-green-refactor cycle, vertical slicing, and uv project setup. Use when writing Python tests, practicing TDD, or starting a uv-managed Python project.
---
...
For Exercism Python tracks that already use pytest without uv, invoke `pytest` directly (for example `pytest -o markers=task <exercise_test.py>`) rather than `uv run`.
...
- Run `uv run pytest` (or the project's test command) after every change
...
- Always use `uv run` to execute commands in a uv project — never activate venvs manually
