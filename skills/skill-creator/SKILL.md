...
Write and edit skills **in the nix-darwin repo**, not in `~/.claude/skills/` or
`~/.cursor/skills/`.
...
- Skip packaging a `.skill` zip unless the user asked for Claude.ai/API upload. The source of truth here is the folder in `skills/`.
- Description-optimizer scripts call the `claude` CLI. If that binary is missing, skip that step; the rest of this skill still applies.
