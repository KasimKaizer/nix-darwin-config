...
This copy is Addy Osmani's `git-workflow-and-versioning` (MIT) with a local overlay.
Upstream: https://github.com/addyosmani/agent-skills/tree/main/skills/git-workflow-and-versioning

When the user asks to open a pull request, follow `creating-pr`. When they ask only for a commit message, follow `writing-commit-messages`.
