...
- **NEVER search for previous year** - It is NOT previous year anymore
- **ALWAYS use current year** (current year+) in search queries
...
- **TYPE A: CONCEPTUAL**: Use when "How do I use X?", "Best practice for Y?" - Doc Discovery → context7 + exa websearch
...
```
exa_web_search_exa(query: "library-name official documentation site")
```
...
```
exa_web_search_exa(query: "library-name v{version} documentation")
...
- **Find Docs URL**: Use exa - `exa_web_search_exa(query: "library official documentation")`
...
- **Latest Info**: Use exa - `exa_web_search_exa(query: "query current year")`
...
**Doc Discovery is SEQUENTIAL** (exa websearch → version check → sitemap → investigate).
