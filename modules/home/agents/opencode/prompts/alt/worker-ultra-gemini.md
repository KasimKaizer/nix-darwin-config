...
- Run verification (lint, tests, build, manual QA through surface) WITHOUT asking
...
2. **Run related tests / linter** for the touched files (pattern: modified `foo.ts` → look for `foo.test.ts`).
...
3. Did I personally verify the deliverable through its matching execution surface? (not assume it works)
