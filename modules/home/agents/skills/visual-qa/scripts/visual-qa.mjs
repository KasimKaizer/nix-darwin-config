...
function pixelsDiffer(ref, refOffset, act, actOffset) {
  const refAlpha = ref[refOffset + 3];
  const actAlpha = act[actOffset + 3];
  if (refAlpha === 0 && actAlpha === 0) return false;
  return (
    refAlpha !== actAlpha ||
...
var PNG_SIGNATURE = Buffer2.from([137, 80, 78, 71, 13, 10, 26, 10]);
...
function parseHeader(data) {
  ...
  if (width === 0 || height === 0) {
    throw new PngDecodeError(
      "invalid PNG dimensions (width and height must be > 0)",
    );
  }
  const bitDepth = data[8] ?? 0;
  const colorType = data[9] ?? 0;
  const compressionMethod = data[10] ?? 0;
  const filterMethod = data[11] ?? 0;
  const interlaceMethod = data[12] ?? 0;

  if (compressionMethod !== 0 || filterMethod !== 0) {
    throw new PngDecodeError("unsupported compression or filter method");
  }
...
function decodePixels(idat, width, height, bpp) {
  ...
  const inflated = inflateSync(idat, { maxOutputLength: expectedBytes });
...
function stringWidth(text, tabWidth = 8) {
  let total = 0;
  for (const char of text) {
    if (char === "\t") {
      total += tabWidth - (total % tabWidth);
      continue;
    }
...
function wideStartColumns(plain, tabWidth = 8) {
  const columns = [];
  let column = 0;
  for (const char of plain) {
    if (char === "\t") {
...
function checkDirectExecution() {
  ...
  try {
    ...
    return entrypointPath === currentFilePath;
  } catch {
