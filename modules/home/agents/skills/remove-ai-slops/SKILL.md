...
In the commands below, `$SKILL_DIR` is this skill's own directory (the folder containing this `SKILL.md`). The bundled Python checker lives at `scripts/check-no-excuse-rules.py` inside it.

---

## Categories (what counts as slop)

The agent looks for these ten categories. The first three are stylistic, the next three are structural, the next two are about hidden cost, the ninth is about behavior coverage, and the tenth is about module sizing.
...
1. **Obvious comments** — comments restating code, trivial docstrings, section dividers, commented-out code, vague TODOs/Notes.
   ...
   - KEEP: BDD markers (`# given`, `# when`, `# then` in Python/Shell; `// given`, `// when`, `// then` in TS/Rust/Go/Nix).
...
3. **Excessive complexity** — deep nesting (>3 levels), nested ternaries, complex boolean expressions (combine 4+ predicates), long parameter lists (>5 args without a struct/dataclass/object), god functions (>50 lines doing many things), overly clever one-liners that sacrifice readability, chains for type/enum/variant discrimination (must use language-idiomatic exhaustive matching: TypeScript `switch` + `assertNever`, Rust `match`, Go `switch v := x.(type)`, Python `match/case` + `assert_never`), loose `object` / `any` used as a type annotation (must use strict typed interfaces, `Protocol`, `TypeVar`, or explicit unions).
   - KEEP: established complexity patterns in this codebase, performance-critical hot paths that intentionally use a complex idiom. Simple if/else for boolean conditions and range checks (not variant discrimination).
   - REFACTOR: nested if-chains → guard clauses / early returns. Complex ternaries → explicit if/else. Variant discrimination → language-idiomatic pattern matching. Loose object/any annotations → strict types/interfaces.
...
8. **Performance equivalences (behavior-preserving optimizations)** — changes that are provably equivalent in semantics but cheaper in time/space:
   ...
   - Expensive or dynamic property evaluation inside loop → hoist outside (do NOT micro-hoist native O(1) `.length` / `len()`)
...
### Sizing
10. **Oversized modules** — any source file exceeding **250 pure LOC** (non-blank, non-comment lines). This is an architectural smell indicating multiple mixed responsibilities. Measure: `awk '!/^[[:space:]]*$/ && !/^[[:space:]]*(#|\/\/)/' <file> | wc -l`.

   **When found in application code, do NOT just flag it. Execute a modular refactoring:**
   1. For Python files, run `python3 "$SKILL_DIR/scripts/check-no-excuse-rules.py"` recursively on scope to list violations.
   2. For each oversized file, identify distinct responsibilities (single-responsibility principle).
   3. Plan the split: name each new file after the concept it owns (never `utils`, `helpers`, `common`, `part_1`).
   4. Present the split plan to the user before executing.
   5. Extract into clean modules following language-idiomatic module conventions (for Python: explicit `__init__.py` re-exports; for TypeScript/Rust/Go/Nix: cohesive module files and standard import/export structures).
   6. Verify: for Python files, run `python3 "$SKILL_DIR/scripts/check-no-excuse-rules.py"` again — every file must be ≤250 pure LOC. Run tests, typecheck, lint.

   **Forbidden escapes**:
   - Counting blanks/comments toward budget.
   - Splitting by token count (`foo_1`, `foo_2`) — split by what each file DOES.
   - Catch-all dump files (`utils`, `helpers`, `service`).
   ...
   KEEP: genuinely self-contained single-responsibility scripts or cohesive declarative configuration tables (e.g., standalone CLI checkers, Nix module maps). Opt out in the first 10 lines with `# noqa: SIZE_OK` (Python/Shell/Nix) or `// noqa: SIZE_OK` (TS/Rust/Go) and a comment explaining why.
...
| Typecheck | `lsp_diagnostics` on changed files / Serena diagnostics (`serena_get_diagnostics_for_file`) + project type-checker | zero new errors |
...
### Phase 0: Plan and track tasks

Create a structured task plan (using `todowrite` in OpenCode or native task tracking in other clients). Mark items in progress one at a time.
...
```bash
BASE_BRANCH=$(git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed 's@^refs/remotes/origin/@@' || (git rev-parse --verify main >/dev/null 2>&1 && echo main) || echo master)
...
**Intentional shortcuts:** if the plan deliberately keeps a bounded simplification (a naive scan fine under N rows, a global lock, an O(n²) path), mark it in-code with a `debt:` comment naming the ceiling and the upgrade trigger (e.g. `// debt: <ceiling>` or `# debt: <ceiling>`), and list it under "Remaining Risks / Deferred" in the report. That section is the debt ledger — a simplification with a known ceiling and no marker is indistinguishable from a bug.
...
Files are processed by `deep` category agents (`worker-deep`) with the `remove-ai-slops` skill loaded, **batched 5 at a time in parallel**. The executable skill name is `remove-ai-slops`. The `deep` category gives the agent enough thoroughness to correctly evaluate the 10 categories and respect the KEEP rules without slipping into surface fixes; the 5-wide batch is the sweet spot — more than 5 creates result-merging noise and context contention, fewer wastes parallelism. (In single-agent environments without subagents, process files sequentially).

**Batching protocol** (strict):

1. Slice the in-scope file list into chunks of up to 5 files.
2. For each chunk, launch all `task` calls **in a single message**.
3. End your turn. Once all in the batch complete, collect and integrate each result.
...
task(
  ...
  description="[deep] Slop removal: {filename}",
  prompt="""
...
LOAD SKILLS: [remove-ai-slops]
"""
)
```

**Batch failure handling**: Mark a file for retry only when the worker completed without the deliverable, failed verification, or is no longer running. Follow up via task session continuation (`task(task_id="ses_...", prompt="...")`). Do NOT block the remaining files in the batch; collect successful results and retry the failed file once later. If retry also fails, escalate that file under "Issues Found & Fixed" in the final report.
...
3. Revert only the problematic change using targeted `Edit` (or `git restore` on only the specific file/hunk touched by the agent), preserving any unrelated worktree changes.
...
- If correctness depends on further inspection, keep using `lsp_diagnostics` or Serena diagnostics (`serena_get_diagnostics_for_file`), the test runner, and direct file reads until the result is grounded.
